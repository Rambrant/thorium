...
// other's way -- AcConfig and its siblings below carried this
...
// "4411". Here the 6834B is a Keysight product; the
...
//
namespace hal::keysight_ac6834b
{
    class Ac6834B;
    ...
    // home of its own: hal::keysight_ac6834b::Ac6834B::measuredNeutralCurrent(), which takes no
    ...
    //
    // Kept as a function in this driver's own namespace rather than calling
    // core::meta::to_string at each site, so an unqualified to_string( phase)
    // resolves by ADL on Phase the way
    ...
    // hal::keysight_ac6834b::Ac6834B on why the phase count is not a setting).
    //
    ...
    // designation, FREQuency does not -- see AcConfig below, where that
    ...
    // hal::keysight_ac6834b::Ac6834B's list of what is not modeled.
    //
    ...
    namespace detail
    {
        ...
        template<typename Symmetry>
        [[nodiscard]]
        constexpr auto asPerPhase( const AcConfig<Symmetry> & config) -> AcConfig<PerPhase>
        {
            return AcConfig<PerPhase>{
                config.Instrument,
    ...
    //
    template<typename Symmetry>
    class AcBuilder : public ConfigBuilder<AcBuilder<Symmetry>, AcConfig<Symmetry>>
    {
        public:
            using Config = AcConfig<Symmetry>;

            explicit AcBuilder( Ac6834B & instrument) :
                AcBuilder::ConfigBuilder( Config{ instrument })
            {}

            // Used by the per-phase setters below to carry an already-built
            // config across the symmetry change.
            explicit AcBuilder( Config config) :
                AcBuilder::ConfigBuilder( std::move( config))
            {}
            ...
            auto phaseVoltage( const PhaseValue<core::quantities::Voltage, Phase::A> a,
                               ...
                               const PhaseValue<core::quantities::Voltage, Phase::C> c) const -> AcBuilder<PerPhase>
            {
                ...
                return AcBuilder<PerPhase>{ widened };
            }

            //
            // One frequency, no per-phase overload -- see AcConfig's own
            ...
            auto currentLimit( const PhaseValue<core::quantities::Current, Phase::A> a,
                               ...
                               const PhaseValue<core::quantities::Current, Phase::C> c) const -> AcBuilder<PerPhase>
            {
                auto widened = detail::asPerPhase( this->mConfig);
    ...
    //     IS modeled (see AcBuilder::range), but only as a setpoint:
    ...
    class Ac6834B : public InstrumentTag
    {
        public:
            ...
            // and nothing else. Deliberately not a Phase (see hal::keysight_ac6834b::Phase):
            ...
            [[nodiscard]]
            auto ac() -> AcBuilder<Balanced>
            {
                return AcBuilder<Balanced>{ *this };
            }
            ...
            // that is likewise scalar-only (see AcConfig).
            //
            [[nodiscard]]
    ...
    //
    template<typename Symmetry>
    auto applyDriver( const AcConfig<Symmetry> & config) -> void
    {
    ...
    template<typename Symmetry>
    auto removeDriver( const AcConfig<Symmetry> & config) -> void
    {
    ...
    //
    template<typename Symmetry>
    auto isEnergised( const AcConfig<Symmetry> & config) -> bool
    {
    ...
    //
    template<typename Symmetry>
    auto describeConfig( const AcConfig<Symmetry> & config) -> core::SourceDescription
    {
    ...
    //
    template<typename Symmetry>
    auto connectDriver( SwitchFabric & fabric, const InstrumentWiring & instrumentWiring, const ConnectorWiring &, const AcConfig<Symmetry> & config) -> void
    {
        fabric.connect( instrumentWiring.findAll( config.Instrument.id()));
    }

    template<typename Symmetry>
    auto disconnectDriver( SwitchFabric & fabric, const InstrumentWiring & instrumentWiring, const ConnectorWiring &, const AcConfig<Symmetry> & config) -> void
    {
