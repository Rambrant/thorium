...
// other's way -- AcquisitionBuilder and its siblings below carried this
...
// "4411". Here the DSO8064A shipped as an Agilent product and is now Keysight; the
...
namespace hal::keysight_dso8064a
{
    ...
    // Same reasoning hal::racal1260::Parity gets, and it matters more here because the
    ...
    // ValidChannel below, which IS a fixed property of the model and
    ...
    // DSO8064A has four physical input channels, so Channel<5> (or
    // <0>) simply has no valid instantiation. Kept as its own concept
    // (rather than inlined into Channel's own template) so the
    ...
    // sensing side and hal::racal1260::Rs232Config on the framing side, and for the
    ...
    struct TriggerConfig
    {
        ...
        // template argument checked against ValidChannel -- the
        // check happens where the number is written, and what survives into
        // the config is the value. Same shape as Channel<N> handing
        // N to setChannel().
        //
    ...
    struct ChannelConfig
    {
        ...
        // filled in by Channel<N>, so a script can only ever produce
        // one of these by naming a channel that ValidChannel accepts.
        //
        unsigned                                Channel;
    ...
    //
    // Separate from ChannelConfig even though both are "a channel and
    ...
    struct WaveformConfig
    {
        ...
        //
        // Not optional, for ChannelConfig::Channel's reason: a
        // transfer with no source is not an underspecified instruction, it is
        // not one at all. Filled in by Channel<N>, so a script can only
        // produce one by naming a channel ValidChannel accepts.
        //
        unsigned    Channel;
    };
    ...
    // hal/driver/builder.hpp) exactly as hal::keysight_n6701a::DcBuilder and
    // hal::racal1260::Rs232Builder do -- so "how do I set X" reads the same way
    ...
    class TriggerBuilder : public ConfigBuilder<TriggerBuilder, TriggerConfig>
    {
        public:
            ...
            explicit TriggerBuilder( DSO8064A & instrument) : ConfigBuilder( Config{ instrument }) {}
    ...
    class TimebaseBuilder : public ConfigBuilder<TimebaseBuilder, TimebaseConfig>
    {
        public:
            ...
            explicit TimebaseBuilder( DSO8064A & instrument) : ConfigBuilder( Config{ instrument }) {}
    ...
    class AcquisitionBuilder : public ConfigBuilder<AcquisitionBuilder, AcquisitionConfig>
    {
        public:
            ...
            explicit AcquisitionBuilder( DSO8064A & instrument) : ConfigBuilder( Config{ instrument }) {}
            ...
            // optional here for the reason AcquisitionConfig gives:
    ...
    // Channel<N>. That is the same dangling-reference lesson
    // core::Port learned (see Channel's own comment below) --
    ...
    class ChannelBuilder : public ConfigBuilder<ChannelBuilder, ChannelConfig>
    {
        public:
            ...
            ChannelBuilder( DSO8064A & instrument, const unsigned channel) :
    ...
    class WaveformBuilder : public ConfigBuilder<WaveformBuilder, WaveformConfig>
    {
        public:
            ...
            WaveformBuilder( DSO8064A & instrument, const unsigned channel) :
                ConfigBuilder( Config{ instrument, channel })
            {}

            //
            // No setters at all, and that is the point rather than an omission
            // -- see WaveformConfig on why a transfer has nothing left
    ...
    class SingleBuilder : public ConfigBuilder<SingleBuilder, SingleConfig>
    {
        public:
            ...
            explicit SingleBuilder( DSO8064A & instrument) : ConfigBuilder( Config{ instrument }) {}
    ...
    // Port referencing Channel<N> itself, which meant a Port
    ...
    class Channel
    {
        public:
            ...
            explicit Channel( DSO8064A & instrument) : mInstrument( instrument) {}

            // --- Configuring this channel: hands off to the builder ---

            [[nodiscard]] auto input( ChannelInput value) const -> ChannelBuilder;
            ...
            [[nodiscard]] auto probeAdapter( ProbeAdapter value) const -> ChannelBuilder;
            ...
            // comment for why Port never references Channel itself.
            //
    ...
    // retirement hal::keysight_l4411a::L4411A gave the old generic hal::Dmm.
    //
    ...
    // hal::keysight_l4411a::L4411A's own Mode-switch pattern. Same accepted sharp edge as
    // hal::keysight_l4411a::L4411A's AC/DC mode: a port handle read after a later mode or
    ...
    //   - Duty cycle, overshoot, preshoot -- see Channel's own
    ...
    class DSO8064A : public InstrumentTag
    {
        public:
            ...
            // instantiation (ValidChannel), a hard compile error,
            // not a runtime range check. See Channel's own comment
            ...
            //
            // Nothing to do -- a scope is passive, exactly as hal::keysight_l4411a::L4411A
            ...
            // called by Channel's builder methods (vpp()/vmax()/.../
            ...
            // hal::racal1260::Racal1260::configurePort makes and for the same reason: an
            ...
            // runs (set by whichever Channel<N> builder method
            // produced the Port), so this needs no channel argument of its
            // own, exactly the same shape hal::keysight_l4411a::L4411A's rawMeasure() has.
            //
    ...
    template<unsigned N>
        requires ValidChannel<N>
    auto Channel<N>::input( const ChannelInput value) const -> ChannelBuilder
    {
        return ChannelBuilder{ mInstrument, N }.input( value);
    }
    ...
    template<unsigned N>
        requires ValidChannel<N>
    auto Channel<N>::probeAdapter( const ProbeAdapter value) const -> ChannelBuilder
    {
        return ChannelBuilder{ mInstrument, N }.probeAdapter( value);
    }
...
#define THORIUM_DSO8064A_PORT( method, quantity, mode)                                                  \
    template<unsigned N>                                                                                \
        requires ValidChannel<N>                                                                \
    auto Channel<N>::method() -> core::Port<core::quantities::quantity, DSO8064A>                \
    {                                                                                                   \
...
#undef THORIUM_DSO8064A_PORT
    ...
    template<unsigned N>
        requires ValidChannel<N>
    auto Channel<N>::riseTime() -> core::Port<core::quantities::Time, DSO8064A>
    {
        mInstrument.setChannel( N);
    ...
    template<unsigned N>
        requires ValidChannel<N>
    auto Channel<N>::fallTime() -> core::Port<core::quantities::Time, DSO8064A>
    {
        mInstrument.setChannel( N);
    ...
    // hal::racal1260::Racal1260's serial port and the way Connect( DcP1.dc()) is on a
    ...
    // script never chose. Same reasoning hal::racal1260::Racal1260's own describeConfig
    ...
    // nothing else in the config (see WaveformConfig on why). What the
