//
// hal::keysight_n6701a::N6701A's own tests, split out of rig/tests/test_source_instruments.cpp
...
namespace
{
    static_assert(   std::constructible_from< hal::keysight_n6701a::Relay,  hal::InstrumentId, hal::Gpib, int> );
    static_assert(   std::constructible_from< hal::keysight_n6701a::Direct, hal::InstrumentId, hal::Lan,  int> );
    static_assert(   std::constructible_from< hal::keysight_n6701a::Relay,  hal::InstrumentId, hal::Usb,  int> );
    static_assert(   std::constructible_from< hal::keysight_n6701a::Relay,  hal::InstrumentId, hal::Simulated, int> );
    static_assert( ! std::constructible_from< hal::keysight_n6701a::Relay,  hal::InstrumentId, hal::Serial, int> );
    static_assert( ! std::constructible_from< hal::keysight_n6701a::Relay,  hal::InstrumentId, hal::Gpib> );
    static_assert( ! std::constructible_from< hal::keysight_n6701a::Relay,  hal::InstrumentId, int> );
} // namespace
...
{
    //
    // hal::keysight_n6701a::N6701A<DirectWiring>'s Isolation doesn't satisfy
    // hal::keysight_n6701a::SwitchableIsolation -- see that concept's own comment for why.
    ...
    concept CanConnectN6701A = requires( hal::SwitchFabric & fabric, const hal::InstrumentWiring & iw, const hal::ConnectorWiring & cw, const hal::keysight_n6701a::DcConfig<Isolation> & config)
    {
        connectDriver( fabric, iw, cw, config);
        disconnectDriver( fabric, iw, cw, config);
    };

    static_assert(  CanConnectN6701A<hal::keysight_n6701a::RelayIsolated> );
    static_assert( !CanConnectN6701A<hal::keysight_n6701a::DirectWiring> );
    ...
    // relay-having tag, never seen anywhere in hal/keysight_n6701a.hpp, still gets
    ...
    struct SourceInstrumentFixture : ::testing::Test
    {
        ...
        hal::keysight_n6701a::Direct      dcP2{ hal::InstrumentId::DcP2, hal::Simulated{}, 2 };
...
TEST( SourceInstrument, DcConnectClosesRemoteSenseLeadsTogetherWithForceWhenTheyAreWired)
{
    ...
    // Sense entry for DcP3). When one does, hal::keysight_n6701a::N6701A's connectDriver
...
{
    // DcP1 and DcP2 are two separate hal::keysight_n6701a::Direct instances -- two
...
{
    hal::keysight_n6701a::Direct dcP1{ hal::InstrumentId::DcP1, hal::Gpib( 0, 14), 1 };
    hal::keysight_n6701a::Relay  dcP3{ hal::InstrumentId::DcP3, hal::Gpib( 0, 14), 3 };
