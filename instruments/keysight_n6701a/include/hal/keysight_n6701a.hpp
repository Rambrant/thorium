...
// other's way -- DcConfig and its siblings below carried this
...
// "4411". Here the N6701A shipped as an Agilent product and is now Keysight; the
...
namespace hal::keysight_n6701a
{
    ...
    // see hal::keysight_n6701a::N6701A's own comment for the real bench fact this models.
    ...
    // to one specific VPC pin for safety reasons (see hal::keysight_n6701a::N6701A's own
    ...
    //
    template<typename Isolation>
    class DcBuilder : public ConfigBuilder<DcBuilder<Isolation>, DcConfig<Isolation>>
    {
        public:
            using Config = DcConfig<Isolation>;

            explicit DcBuilder( N6701A<Isolation> & instrument) :
    ...
    // own output, so one hal::keysight_n6701a::N6701A instance models one module/channel --
    ...
    // hal::keysight_dso8064a::DSO8064A has. Which DUT point that fixed channel corresponds to
    ...
    // by hal::keysight_l4411a::L4411A and hal::keysight_dso8064a::DSO8064A respectively): those two were generic
    ...
    class N6701A : public InstrumentTag
    {
        public:
            ...
            //
            [[nodiscard]]
            auto dc() -> DcBuilder<Isolation>
            {
                return DcBuilder<Isolation>{ *this };
            }
    ...
    // question entirely.
    //
    using Direct = N6701A<DirectWiring>;
    using Relay  = N6701A<RelayIsolated>;
    ...
    //
    template<typename Isolation>
    auto isEnergised( const DcConfig<Isolation> & config) -> bool
    {
    ...
    //
    template<typename Isolation>
    auto describeConfig( const DcConfig<Isolation> & config) -> core::SourceDescription
    {
    ...
    // path to look up any more (see DcConfig's own comment), so
    ...
    auto connectDriver( SwitchFabric & fabric, const InstrumentWiring & instrumentWiring, const ConnectorWiring &, const DcConfig<Isolation> & config) -> void
    {
    ...
    template<typename Isolation>
        requires SwitchableIsolation<Isolation>
    auto disconnectDriver( SwitchFabric & fabric, const InstrumentWiring & instrumentWiring, const ConnectorWiring &, const DcConfig<Isolation> & config) -> void
    {
