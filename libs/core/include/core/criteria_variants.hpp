...
// (see core/active_criteria.hpp for how, and suite/README.md for why), so the
...
//
// Deliberately a separate header from core/active_criteria.hpp, and it pulls in
...
// this is included by main.cpp and core/src/journal.cpp, neither of which has
...
// no parameters at all (see core/test_catalog.hpp on why), so there is no route
