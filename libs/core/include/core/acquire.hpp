...
namespace core
{
    ...
    // to writeDriver/readDriver (see core/transfer.hpp's own comment on the
    // same choice).
    //
    // ---------------------------------------------------------------------
    ...
    // event. That is the same shape as core/source.hpp's Connect-before-Apply
    ...
    // core/transfer.hpp). An Arm is stimulus: it goes straight to the
    ...
    // See core::SessionBank's own comment, and hal/measure.hpp for where this
    ...
    class ArmEngine
    {
        public:
            ...
            auto operator()( const BuilderT & builder) const -> void
            {
                ...
                // core/source.hpp gives: if arming failed, nothing is armed and
