...
namespace core
{
    namespace
    {
        ...
        // see core/recording.hpp on why a collision here is caught rather than
        ...
        // core/recording.hpp for the encoding and why it is binary.
        //
        [[nodiscard]]
    ...
    auto RecordingWriter::spill( const Bytes & payload, const std::string_view extension) -> std::string
    {
        ...
        if( std::filesystem::exists( path))
        {
            ...
            // must not be written through. See core/recording.hpp.
            //
    ...
    auto RecordingWriter::valueColumn( const Bytes & payload, const std::string_view extension) -> std::string
    {
        ...
        // file once it is not -- see core/recording.hpp on why the boundary
    ...
    auto RecordingWriter::write( const RecordedSample & sample) -> void
    {
        ...
        else if( const auto * trace = std::get_if<Waveform>( &sample.mValue))
        {
            ...
            // in core/recording.hpp on why they are in the row rather than in
            // the sample file.
            //
        ...
        const std::filesystem::path &  sidecarDirectory) -> std::vector<RecordedSample>
    {
        ...
        while( std::getline( in, line))
        {
            ...
            // is standing in for. See core/recording.hpp.
            //
            ...
            // the samples -- see kTraceKind in core/recording.hpp on why they
            ...
            const auto octetsOf = [&]( const std::string & column) -> Bytes
            {
                if( !column.empty() && column.front() == kSidecarMarker)
                {
                    ...
                    // recording rather than in it -- see core/recording.hpp.
                    //
