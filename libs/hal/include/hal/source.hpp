...
// core/source.hpp's own comment for why routing moved out from under Apply),
// so unlike hal/route.hpp's Connect/Disconnect and hal/measure.hpp's
// MeasureEngine alias, none of these aliases takes a template argument.
//
// That is also why this header includes neither hal/switch_fabric.hpp nor
// hal/wiring.hpp: nothing declared here has any idea the switching fabric
// exists.
//
...
// see hal/route.hpp for the framing, and core/source.hpp for why the relay
// should move while the path is dead.
//
// Defined once in hal/source.cpp, which needs nothing from the linking rig --
// see that file, and hal/route.hpp for the two verbs that do.
//
