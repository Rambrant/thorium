...
namespace hal
{
    ...
    // core/interlock.hpp, which is where the argument for refusing that one
    ...
    // core/measure.hpp free of any hal:: dependency: the caller only ever puts
    ...
    // at hal/src/interlock.cpp on purpose (see hal/measure.cpp and
    // hal/route.cpp, which are the same shape for the same reason, and
    // libs/hal/CMakeLists.txt on why that is a target boundary rather than a
    ...
    // hal/wiring.hpp, next to the SourceWiring table it consults, because it
