...
namespace hal
{
    ...
    //     as a list; see hal/src/verbs/safing.cpp.
    //
    ...
    // --safe (see framework/runner/src/main.cpp), which calls this and exits. A fresh
    ...
    // on a kind mismatch; see core/quantities/quantity_kind.hpp) -- as opposed to the
    // abnormal one safeRig() itself exists for: a supervising process
    // re-invoking this binary with --safe after the previous one is
    // already dead and can't be unwound at all (see framework/runner/src/main.cpp).
    //
