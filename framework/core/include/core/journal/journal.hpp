...
namespace core
{
    ...
    //     server to ingest (core/journal/sarif_sink.hpp)
    //   - core::RtfSink / core::ConsoleSink -- Measure and Verify only,
    //     colour-coded, for a person (core/journal/rtf_sink.hpp, core/journal/console_sink.hpp)
    //
    ...
    // logger one (see core/catalog/test_catalog.hpp on why), and the verbs
    // a script calls -- Measure, Apply, Verify -- are themselves globals for
    // the same reason (see hal/src/verbs/measure.cpp, hal/src/verbs/source.cpp). A journal
    ...
    // guard destructor or a --safe re-invocation (see hal/verbs/safing.hpp). It is
    ...
    // communication (see core/verbs/transfer.hpp). Each is a genuinely distinct thing
    ...
    // in core/criteria/verify.hpp). Everything that already tells a failed check from a
    ...
    enum class Verb
    {
        ...
        Read,
        //
        // The triggered-acquisition pair (see core/verbs/acquire.hpp). Two verbs
        ...
        //
        // Taking a captured trace off an instrument (see core/verbs/trace.hpp). An
    ...
    struct RunInfo
    {
        ...
        //
        // Distinct from FrameworkVersion, which versions framework/ -- the code that
        ...
        // core::Bench in core/session/bench.hpp.
        //
    ...
    // compile definitions framework/core/CMakeLists.txt sets, plus the current UTC
    ...
    // (see framework/runner/src/main.cpp) to fill in, since neither is derivable here.
    //
    [[nodiscard]]
    ...
    // core/src/journal/report.cpp) and two placeholders of different widths would
    ...
    // byte count (see kMaxDescribedBody in core/quantities/bytes.hpp). This is the
    ...
    // full fidelity, which is what --replay reads (see core/session/recording.hpp).
    //
    ...
    // but no Unit. See core/quantities/format.hpp for what produces them.
    //
    ...
    struct JournalEvent : JournalRecord
    {
        ...
        // column for exactly these events (see core/session/recording.hpp and
    ...
    class IJournalSink
    {
        public:
            ...
            // see framework/runner/src/main.cpp, which is where the selection is known.
            //
    ...
    class Journal
    {
        public:
            ...
            // use of them because the caller holds them (see framework/runner/src/main.cpp,
            ...
            // bracketing work", not "did the DUT pass" (see framework/runner/src/main.cpp).
            //
            ...
            // now (see core/catalog/test_catalog.hpp); this is the one place the
            // verdict is decided.
            //
