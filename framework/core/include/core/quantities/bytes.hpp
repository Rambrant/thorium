...
namespace core
{
    ...
    // core/session/session.hpp for the parallel seam this travels through, which
    ...
    //
    // A non-template overload rather than another branch in core/quantities/format.hpp's
    // describeValue chain, so core/quantities/format.hpp keeps knowing nothing about this
    ...
    // constexpr` member (see core/criteria/criterion.hpp), so an expected value has to
    ...
    // itself lives in the recording (see core/session/recording.hpp), which stores
    ...
    // to every value from every verb (see core/journal/journal.hpp).
    //
    ...
    namespace quantities
    {
        ...
        // spelling it reaches EQ, MASK and _V by (see core/criteria/active_criteria.hpp,
        // which does `using namespace core::quantities`):
        //
        //     CRIT( FS_Console_Ack, EQ( bytes( "ACK")), "...")
        //
        // Same reason core/criteria/predicates.hpp reopens it rather than inventing a
