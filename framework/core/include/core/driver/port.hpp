...
namespace core
{
    ...
    // always catches this (see core/verbs/measure.hpp), so it never escapes a
    ...
    // at(...)) call still works, no fields required. See core/driver/port.hpp for
    ...
    // and an id() -- hal::L4411A/hal::DSO8064A/etc (see hal/driver/instrument.hpp,
    ...
    class Port
    {
        public:
            ...
            // core/criteria/predicates.hpp, so a criterion checked against one fails,
