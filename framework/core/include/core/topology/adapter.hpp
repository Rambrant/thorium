...
namespace core
{
    ...
    // (see hal/topology/adapter.hpp's SOURCE_POINT, and hal::SourceWiring for the
    ...
    // against a misspelled criterion id -- see hal/topology/adapter.hpp's ADAPTER/POINT
    ...
    // (see hal/topology/vpc_location.hpp) is simply the concrete coordinate type this
    ...
    // session (see core/session/session.hpp) has something to key a recording or an
    ...
    // three-phase input. See hal/topology/adapter.hpp's BUNDLE/LINE macros, which
    // build these.
    //
    // An empty, non-virtual marker base, exactly like hal::InstrumentTag
    // (hal/driver/instrument.hpp) and for the same reason: it costs nothing, and it
    ...
    // hal::bundleLocations in hal/topology/bundle.hpp).
    //
    ...
    // hal/topology/adapter.hpp's BUNDLE macro. C++ lets a variable hide a class name
