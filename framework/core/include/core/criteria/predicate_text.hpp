...
//
// Deliberately not in core/criteria/predicates.hpp, even though these overloads describe
...
// split, and the same reasoning, as core/quantities/format.hpp being separate from
// core/quantities/quantity_kind.hpp.
//
...
namespace core::quantities
{
    ...
    // core/criteria/predicates.hpp. A report saying "bits 0x0C set" is checkable against
    ...
    //
    // NE( x) is NotPredicate{ EQ( x)} (see core/criteria/predicates.hpp on why it isn't
    ...
    // NotPredicate{ EQ( ...)} (see core/criteria/predicates.hpp), and gets its own
