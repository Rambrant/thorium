...
namespace core
{
    ...
    namespace detail
    {
        ...
        // core/journal/journal.hpp's JournalRecord). All three are produced by
        // core/quantities/format.hpp from whatever type T the criterion's predicate
    ...
    // core/criteria/criterion.hpp and core/criteria/active_criteria.hpp). These overloads are what
    ...
    // exactly what core/criteria/predicates.hpp refuses to let an empty ANY_OF() become
    ...
    //
    // What a CRIT in a merged table actually is (see core/criteria/criterion.hpp), so
