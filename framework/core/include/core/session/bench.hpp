...
namespace core
{
    ...
    // then asserts on (see framework/hal/tests, rig/tests). Deriving detachment from
    ...
    // whole run -- framework/runner/src/main.cpp -- is in a position to say the second one.
    ...
    // something (see isHumanRelevant in core/src/journal/report.cpp), so this marker
