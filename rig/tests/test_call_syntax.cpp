...
TEST_F( CallSyntaxFixture, MeasureAcceptsTheChainedThresholdBuilderDirectlyAtTheCallSite)
{
    ...
    osc1.setSimulatedRiseTime( 2, Time{ 9e-9});

    const auto reading = Measure( osc1.channel<2>().riseTime().lowThreshold( 0.2).highThreshold( 0.8), at( ClockOut));
